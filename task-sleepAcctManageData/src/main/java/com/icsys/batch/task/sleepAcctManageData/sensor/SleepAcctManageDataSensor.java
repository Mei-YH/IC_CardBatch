package com.icsys.batch.task.sleepAcctManageData.sensor;
...
public class SleepAcctManageDataSensor implements Sensor {
	private Logger LOG = Logger.getLogger(SleepAcctManageDataSensor.class);
	...
	public JobContext[] detect(JobContext context) throws JobException {
		...
		try {
			...
			cycle = SystemParamValue.getIcSysPara("100002");// 无业务间隔时间
		...
		context.setAttribute("cycle", iCycle);
		...
		try {
			...
			if (fileList.size() > 0 && null != fileList) {// 如果返回有处理的文件
				...
				for (File f : fileList) {
					if(f.length() > 0){
						FileUtils.copyFile(f, new File(localPath + File.separator + date, f.getName()));
						JobContext jobContext = new JobContext();
						jobContext.setAttribute("fileName", new File(localPath + File.separator + date, f.getName()).getAbsolutePath());
						jobs.add(jobContext);
					}
					f.delete();
					new File(f.getParent(), f.getName() + ".CHK").delete();
				}
