 ...
 * 
 */
public class OfflineCon300Step implements Step {
	private static Logger LOG = Logger.getLogger(OfflineCon300Step.class);
	...
	public long estimate(JobContext context) throws JobException {
		...
		int count = clearBeanService.getTotalCount(context.getJobDate().trim(), context.getJobNo().trim(), "300");
	...
	public void perform(JobContext context, Executor executor)	throws JobException {
		try {			    
			...
			for (int pageNo = start; pageNo <= end; pageNo++) {
				// 每个线程负责一页
				OfflineTranProxy proxy = new OfflineTranProxy(context,"300",pageNo);
				executor.execute(proxy);
			}
//			LOG.info("脱机消费处理成功...");
		} catch (Exception e) {
			throw new JobException("脱机消费处理异常,任务挂起,原因:" + e.getMessage());
		}		
	}
